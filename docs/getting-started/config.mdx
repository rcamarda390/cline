...
Set `cline.offlineMode` to `true` in VS Code settings when the extension must run without automatic network activity. Offline mode is local to the extension and does not require remote configuration or DNS access; it skips startup authentication checks, remote configuration refreshes, model catalog refreshes, marketplace refreshes, and telemetry provider initialization. Cline can still use locally configured or reachable provider endpoints when you explicitly use them.
