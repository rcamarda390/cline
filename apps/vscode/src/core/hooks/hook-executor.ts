...
async function updateHookMessage(
	...
	const hookMessageIndex = clineMessages.findIndex(
		(m: ClineMessage) => m.ts === hookMessageTs,
	);
	if (hookMessageIndex !== -1) {
		...
		await sendPartialMessageEvent(
			convertClineMessageToProto(clineMessages[hookMessageIndex]),
		);
	}
}
...
async function reorderHookAndToolMessages(
	...
	for (let i = lastToolMessageIndex + 1; i < clineMessages.length; i++) {
		if (
			clineMessages[i].say === "hook_status" ||
			clineMessages[i].say === "hook_output_stream"
		) {
